...
class NoteControllerTest extends RestDocsTest {
    ...
    void 아티스트와_관련된_노트_리스트를_조회하면_데이터와_200응답을_해야_한다() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/notes/artists")
                        ...
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andDo(getNoteListByArtistDocument());
    }
    ...
    @Test
    void 곡과_관련된_노트_리스트를_조회하면_데이터와_200응답을_해야_한다() throws Exception {
        // given
        User user = UserFixture.create();
        ...
        // when, then
        mockMvc.perform(get("/api/v1/notes/songs")
                        .header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken)
                        .param("songId", "1")
                        .param("hasLyrics", "false")
                        .param("cursor", "1")
                        ...
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andDo(getNoteListBySongDocument());
    }
    ...
    private RestDocumentationResultHandler getNoteListBySongDocument() {
        ...
        return restDocs.document(
                resource(ResourceSnippetParameters.builder()
                        ...
                        .responseSchema(Schema.schema("Note List By Song Response"))
                        .build()
                )
        );
    }
