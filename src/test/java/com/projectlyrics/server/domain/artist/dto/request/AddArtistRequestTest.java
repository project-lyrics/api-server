...
class AddArtistRequestTest {
  ...
  @ParameterizedTest
  @ValueSource(strings = {"", "     ", "넬"})
  void 아티스트의_영어_이름이_영어_이외_문자열이나_빈_문자열_또는_공백_문자열이라면_validation_에러가_발생한다(String englishName) {
    // given
    ...
    // then
    violation.forEach(error -> {
      assertThat(error.getMessage()).isEqualTo("영어 외의 문자열, 빈 문자열 또는 공백 문자열은 허용하지 않습니다.");
    });
  }
